...
Menu m;
int connectorColour = color(0, 255, 0);
...
class darkButton extends Button {
  int iconColour, outline = color(0), litOutline = color(255), buttonColour = color(28, 56, 250);
  float buffer = 5;
  darkButton(String text, float xTemp, float yTemp, float wTemp, float hTemp, int colour, int xc) { // button colour, X colour
    super(text, xTemp, yTemp, wTemp, hTemp, colour, xc);
    iconColour = xc;
  }
...
class Menu {
  ...
  float size1 = 32.3f, size2 = 23.2f, size3 = 18;
  ...
  Menu() {
    ...
    Q = new Button("?", w/2 - menuWidth/2 + xSize/2 + 0.5f, h/2 - menuHeight/2 + xSize/2 + 0.5f, xSize, xSize, yellow, size3);
  ...
  public void move(int tw, int th) {  // w and h are screen width and height or wherever to move the menu
    ...
    Q.move(w/2 - menuWidth/2 + xSize/2 + 0.5f, h/2 - menuHeight/2 + xSize/2 + 0.5f);
...
class Node extends VerletParticle2D {
  int highlightColour = color(255, 255, 0);
  ...
  public String convertGreek(String t) {
